...
private:
	...
	/**
	 * Set true when BarbequeRTRM should terminate
	 */
	bool done;
	 ...
	 *
	 * The platform services are a set of services exported by BarbequeRTRM
	 * to other modules (and plugins). The resource manager ensure an
	...
	/**
	 * @brief The map of registered Worker
	 */
	 ...
	 *
	 * This is used to collect and aggregate optimization requests.
	 ...
	 * fast discharging rate, set resources offline, degradation notifications,...)
...
#define BBQUE_DEFAULT_RESOURCE_MANAGER_OPT_INTERVAL 0
	 ...
	 * and an eventual Synchronization of the active applications
	 ...
	 * This provides the BarbequeRTRM applications and resources control logic.
	 * This is actually the entry point of the BarbequeRTRM state machine.
	 */
	void ControlLoop();
